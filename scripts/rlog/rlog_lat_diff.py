...
соглашении CAN с уже применённым `vehicle.steer_sign`, поэтому скрипт мерит связь, а не зашивает её.
  ...
  OPENPILOT_ROOT=/path/to/openpilot python3 rlog/rlog_lat_diff.py <route dir> [<route dir> ...]
  python3 rlog/rlog_lat_diff.py <parent of routes> --reference model
  python3 rlog/rlog_lat_diff.py <route> --segments 6 --controller fp --steer-ratio 16.12
"""
    ...
    def _cereal():
        ...
        if not root:
            raise SystemExit(
                ...
                "  OPENPILOT_ROOT=/path/to/openpilot python3 rlog/rlog_lat_diff.py <route dir>"
            )
