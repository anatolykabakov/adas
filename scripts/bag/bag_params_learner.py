...
same construction as `bag/bag_road_roll.py`: mount frame from gravity and braking, then
...
speed variety separates them. On one stretch at one speed it cannot.
  ...
  python3 bag/bag_params_learner.py adas_logs/2026_08_06_00_36_42
  python3 bag/bag_params_learner.py adas_logs/* --no-roll
  python3 bag/bag_params_learner.py adas_logs/2026_08_06_00_36_42 --speed-band 12 26
"""
...
def mount_frame(accel, v_at_imu, a_at_imu):
    """Phone axes -> vehicle axes (x fwd, y right, z down). Same construction as bag/bag_road_roll.py."""
    still = v_at_imu < 0.2
    if still.sum() < 50:
        return None
