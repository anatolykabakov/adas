...
`blend`/shift or a different version of `laneLinesToPath`), and the numbers cannot be trusted.
  ...
  python3 bag/bag_arc_offset.py adas_logs/<run>
  python3 bag/bag_arc_offset.py adas_logs/<run> --plots /tmp/adas_plots --prefix 0802
  python3 bag/bag_arc_offset.py adas_logs/<run> --cache /tmp/run.npz   # second run is instant
  python3 bag/bag_arc_offset.py adas_logs/<run> --weight-by-std        # how σ weight behaves
...
the wrong reference is restored, and the CTE self-check will show it:
  ...
  python3 bag/bag_arc_offset.py adas_logs/<run> --blend 0.6 --weight-by-std \
