// Раннер thneed: исполнение записанной очереди OpenCL-ядер на GPU телефона.
//
// Строки ниже до отметки «наш JNI» — код flowpilot (MIT), файл
// `android/src/main/cpp/jniconvert.cpp`, взятый почти дословно: реализация Thneed, ThneedModel и
// биндингов OpenCL через dlopen. Их собственный JNI отброшен — он публикует в cereal, которого у нас
// нет, и завязан на их имена пакетов.
//
// Что такое thneed, одной строкой: НЕ рантайм, а сериализованная запись последовательности ядер с
// аргументами (`vector<CLQueuedKernel>`), которую `load()` читает, а `clexec()` проигрывает. Ядра лежат
// СКОМПИЛИРОВАННЫМИ БИНАРНИКАМИ и грузятся через `clCreateProgramWithBinary`, поэтому файл привязан к
// GPU и драйверу, которые его собрали. Из этого следует: свою сетку так просто не подсунешь — её надо
// компилировать на устройстве через tinygrad (`openpilot/compile2.py`). Здесь мы исполняем ИХ модель.
//
// Подробности и план — docs/VISION_RATE.md §5.
...
// Загрузка OpenCL: по короткому имени не проходит.
//
// Проверено на OnePlus 7T (Android 12, Adreno 640): `dlopen("libOpenCL.so")` возвращает NULL, хотя
// libOpenCL.so перечислен и в /vendor/etc/public.libraries.txt, и в системном списке. У flowpilot
// написано именно короткое имя — видимо, их APK собран под другой targetSdk, где вендорные публичные
// библиотеки видны иначе. Поэтому перебираем кандидатов и пишем, какой сработал: имя библиотеки — не то,
// что стоит зашивать вслепую под чужой телефон.
static void *load_opencl() {
    ...
    for (const char *name : candidates) {
        dlerror();  // сбросить прошлую ошибку, иначе dlerror() отдаст чужую — на этом я уже обжёгся:
                    // он вернул "undefined symbol: Java_..._nativeInit___3BI" от разрешения JNI.
...
ThneedModel::ThneedModel(uint8_t* model, float *_output, size_t _output_size, int runtime, bool luse_tf8, cl_context context) {
...
// ======================== наш JNI ========================
//
// Отличия от их версии, каждое по делу:
//   * нет cereal: выход отдаётся в Java, публикацией занимается наш конвейер;
//   * состояние (features_buffer, prev_desired_curvs) живёт здесь же, как у них, — Java им не управляет;
//   * имена пакетов наши.
//
// Раскладка плоского входного буфера, повторяет flowpilot:
//   [0                     .. IMG_LEN)          input_imgs      (12x128x256 float)
//   [IMG_LEN               .. 2*IMG_LEN)        big_input_imgs  (то же, warp широкой матрицей)
//   [2*IMG_LEN             .. +DESIRE_LEN)      desire          (100x8)
//   [2*IMG_LEN+DESIRE_LEN  .. +2)               lateral_control_params: vEgo, задержка актюатора
//
// Сырой выход модели: OUTPUT_SIZE разбираемых значений плюс FEATURE_LEN фич, которые уходят обратно во
// вход следующего кадра. Java получает всё целиком.

// Размеры модели. У flowpilot они приходят из `selfdrive/modeld/models/driving.h`, которого в их же
// Android-наборе нет — там эти строки просто закомментированы, а значения подставляются их сборкой.
// Объявляем явно, потому что зависеть от инклюда, которого не видно, хуже, чем записать число и указать
// источник. Значения: driving.h:17-19 и вычисление OUTPUT_SIZE = sizeof(ModelOutput)/sizeof(float),
// которое на модели f3 даёт 5992; NET_OUTPUT_SIZE = OUTPUT_SIZE + FEATURE_LEN = 6504 — столько Java и
// выделяет под выход.
...
// Проверка ДО первого вызова OpenCL, а не полагание на try/catch в Java.
//
// Урок с телефона: при неразрешённых указателях `cl_get_device_id` падает по SIGSEGV, а нативный сигнал
// Java-обработчиком не ловится — умирает весь процесс. То есть обещанный «откат на ONNX» без этой
// проверки не работал: отдельная .so защищает от ошибки ЗАГРУЗКИ, но не от разыменования null внутри.
...
Java_ai_flow_adas_vision_SupercomboThneedRunner_nativeInit(JNIEnv *env, jclass, jbyteArray modelData, jint outLen) {
    if (!openclReady()) {
        return JNI_FALSE;
    }
    if (thneed != NULL) {
        // Модель уже загружена, но рекуррентное состояние надо обнулить: раннер пересоздаётся при
        // смене модели на ходу, и Java-сторона начинает с пустой пары кадров, а здесь иначе остались
        // бы 99 кадров фич и кривизны от прошлой сессии — несогласованное состояние на первых кадрах.
        for (int i = 0; i < features_len; i++) features_buf[i] = 0;
        for (int i = 0; i < PREV_DESIRED_CURVS_LEN; i++) prev_curvs_buf[i] = 0;
    ...
    for (int i = 0; i < zero_len; i++) zero_buf[i] = 0;
    for (int i = 0; i < features_len; i++) features_buf[i] = 0;
    for (int i = 0; i < PREV_DESIRED_CURVS_LEN; i++) prev_curvs_buf[i] = 0;
...
Java_ai_flow_adas_vision_SupercomboThneedRunner_nativeExecute(JNIEnv *env, jclass, jfloatArray input,
                                                              jfloatArray output) {
    if (thneed == NULL) return -1.0f;
    ...
    // Рекуррентность: сдвинуть историю на кадр и дописать свежие фичи из хвоста выхода.
    ...
    // История заданных кривизн — модель 0.9.x получает свою прошлую команду обратно на вход.
    //
    // РАСХОЖДЕНИЕ С flowpilot, сознательное. У них здесь memmove по БАЙТАМ:
    // `PREV_DESIRED_CURVS_LEN - 1` вместо `sizeof(float) * (...)`, то есть сдвигается четверть
    // истории, а на границе остаётся полузаписанный float. Это вход рекуррентной модели, и кормить
    // его мусором нельзя — тем более что качество разметки на thneed у нас пока не измерено вовсе,
    // и такой вход был бы первым подозреваемым. Сдвигаем по элементам.
