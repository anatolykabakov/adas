...
/*
 * Linux UAPI header for the Qualcomm KGSL driver, vendored from flowpilot
 * (third_party/linux/include/msm_kgsl.h). Needed by thneed_runner.cpp to submit
 * command batches to the Adreno GPU directly.
 */
...
/* device id */
enum kgsl_deviceid { KGSL_DEVICE_3D0 = 0x00000000, KGSL_DEVICE_MAX };
...
#define KGSL_MEMSTORE_OFFSET(ctxt_id, field)                                                                           \
  ((ctxt_id) * sizeof(struct kgsl_devmemstore) + offsetof(struct kgsl_devmemstore, field))
...
#define IOCTL_KGSL_CMDSTREAM_FREEMEMONTIMESTAMP_CTXTID                                                                 \
  _IOW(KGSL_IOC_TYPE, 0x17, struct kgsl_cmdstream_freememontimestamp_ctxtid)
