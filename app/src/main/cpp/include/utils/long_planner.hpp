 ...
 *  only actuator is the cruise set speed (`PandaService::computeCruiseButtons` taps GRA +/−), and
 ...
 *
 *  Split out of `LongPlanService` so the rules can be tested without a middleware instance, the
 *  same way `safety_planner.hpp` is split out of `SafetyWarnService`. */
struct Config {
