...
class Collector : public adas::Service {
public:
...
class Injector : public adas::Service {
public:
...
/** `Middleware::step()` drains inboxes before firing timers, so a message published from a timer reaches
 *  its subscriber on the *next* step. Every assertion here is about what a timer published, so always pump
 *  twice. */
void pump(adas::Middleware& bus, uint64_t t_us)
{
  bus.setTime(t_us);
  bus.step();
  bus.step();
...
// it.
TEST(MapDataService, PublishesMatchedRouteFromGpsAndPose)
{
  if (!haveMap())
    GTEST_SKIP() << "no map file at " ADAS_TEST_MAP_FILE;

  adas::Middleware bus(adas::Middleware::Mode::Simulated);

  adas::MapDataService::Config cfg;
  ...
  auto svc = bus.registerService<adas::MapDataService>(cfg);
...
TEST(MapDataService, LoggedMessageStaysSmallEnoughToRecord)
{
  if (!haveMap())
    GTEST_SKIP() << "no map file at " ADAS_TEST_MAP_FILE;
  ...
  bus.registerService<adas::MapDataService>(cfg);
  auto inj = bus.registerService<Injector>();
  auto col = bus.registerService<Collector>();
  ...
  adas::Middleware bus2(adas::Middleware::Mode::Simulated);
  cfg.local_map_period_s = 0.0;  // attach it to every message
  cfg.local_map_radius_m = 400.0;
  bus2.registerService<adas::MapDataService>(cfg);
...
TEST(MapDataService, PublishesUnmatchedWithoutAFix)
{
  if (!haveMap())
    GTEST_SKIP() << "no map file at " ADAS_TEST_MAP_FILE;
...
TEST(MapDataService, StopsMatchingWhenTheFixGoesStale)
{
  if (!haveMap())
    GTEST_SKIP() << "no map file at " ADAS_TEST_MAP_FILE;
