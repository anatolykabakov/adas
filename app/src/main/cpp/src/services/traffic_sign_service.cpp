...
void TrafficSignService::configure()
{
  ...
  scheduleTimer(
      100, [this] { tick(); }, "tick");
  LOGI("TrafficSignService: %s + chassis → %s (speed limit / TFL / overspeed)", topics::kTrafficDetections,
       topics::kTrafficVision);
}

void TrafficSignService::reset()
{
  have_chassis_ = false;
  chassis_ = {};
  speed_limit_kmh_ = 0;
...
void TrafficSignService::onChassis(const ChassisSample& msg)
{
  chassis_ = msg;
  have_chassis_ = true;
}

void TrafficSignService::onDets(const ai::flow::adas::ZMQMessage& msg)
{
...
void TrafficSignService::publishState(int64_t now_ms)
{
