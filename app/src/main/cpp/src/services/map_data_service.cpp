...
std::vector<std::string> mapPathCandidates(const std::string& path)
{
  ...
  return {
      path,
      "/sdcard/adas_maps/" + path,
      "/storage/emulated/0/adas_maps/" + path,
      "maps/" + path,
      "../maps/" + path,
      "../../../../maps/" + path,
  };
...
MapDataService::MapDataService(Config config) : config_(std::move(config)) {}

bool MapDataService::loadMap()
{
  if (config_.map_path.empty()) {
    LOGI("MapDataService: map_path empty — service idle");
    return false;
  }
  for (const auto& candidate : mapPathCandidates(config_.map_path)) {
    ...
    LOGE("MapDataService: %s exists but did not load", candidate.c_str());
  }
  LOGE("MapDataService: map '%s' not found — service idle. On the phone this means the asset was not "
       "unpacked (is `map.path` the asset's name?); elsewhere, put it in maps/ or /sdcard/adas_maps/",
       config_.map_path.c_str());
  return false;
}

void MapDataService::configure()
{
  map_loaded_ = loadMap();

  subscribe<ai::flow::adas::ZMQMessage>(topics::kGpsData, [this](const ai::flow::adas::ZMQMessage& m) { onGpsData(m); });
  ...
  LOGI("MapDataService → %s  map=%s (%zu edges) %.1f Hz horizon=%.0fm step=%.1fm window=%.0fm "
...
void MapDataService::onGpsData(const ai::flow::adas::ZMQMessage& msg)
{
  ...
  if (have_anchor_ && have_pose_) {
    ...
    if (last_gap_m_ > config_.max_pose_gap_m)
      LOGW("MapDataService: pose drifted %.0f m from GPS between fixes — re-anchoring", last_gap_m_);
  }
...
void MapDataService::onPose(const ai::flow::adas::ZMQMessage& msg)
{
...
bool MapDataService::currentPosition(double& x, double& y, double& yaw) const
{
  if (!have_anchor_)
    return false;
...
void MapDataService::fillLocalMap(ai::flow::adas::MapLocalState& out, double x, double y)
{
...
void MapDataService::onTick()
{
  ...
  if (++ticks_ % 600 == 0) {
    LOGI("MapDataService: %llu ticks, matched %.0f%%, last match %.1f m on '%s', build %.2f ms",
         static_cast<unsigned long long>(ticks_), 100.0 * static_cast<double>(matched_ticks_) / static_cast<double>(ticks_),
         route_.match_dist_m, route_.road_name.c_str(), build_us / 1000.0);
  }
}
