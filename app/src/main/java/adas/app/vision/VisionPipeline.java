...
public class VisionPipeline {
    ...
    public VisionPipeline(Context context, LaneOverlayView overlay, boolean cameraCalib)
            throws Exception {
        this(context, overlay, cameraCalib, adas.app.AdasConfig.modelRunner(context));
    }
    ...
    public void submitBitmap(Bitmap bitmap) {
        submitBitmap(bitmap, adas.app.TimeUtil.nowMs());
    }
    ...
    public void submitBitmap(Bitmap bitmap, long captureTsMs) {
        ...
        final long captureTs = captureTsMs > 0 ? captureTsMs : adas.app.TimeUtil.nowMs();
        handler.post(() -> {
    ...
    public void submitYuv(YuvFrame frame, long captureTsMs) {
        ...
        final long captureTs = captureTsMs > 0 ? captureTsMs : adas.app.TimeUtil.nowMs();
        final long submitTs = adas.app.TimeUtil.nowMs();
        boolean start;
        synchronized (lock) {
            if (pendingYuv != null) {
    ...
    private void drainYuvLatest() {
        while (true) {
            ...
            final long pickupTs = adas.app.TimeUtil.nowMs();
            final int id = frameId++;
            SupercomboOnnxRunner.Result res = null;
            try {
