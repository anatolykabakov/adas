...
Pure Pursuit выбирает **один** $\delta$ — тот, что ведёт в точку прицеливания.
**MPC** оптимизирует **короткий отрезок будущей траектории**, применяет первую команду и пересчитывает всё заново на следующем кадре.
...
Здесь мы разберём MPC на **игрушечной модели в области пути — такой, что целиком укладывается в голове**;
она уместилась в семь шагов ниже. Эта конструкция рулила как контроллер `vp` / VisionPilot вплоть до
2026-08-21, пока `fp` не вытеснил её с дороги и её не убрали; код остался в истории git, а урок — здесь,
потому что это самый наглядный способ понять, что вообще делает любой MPC. [Следующая
глава](./MPC_and_FP.md) посвящена **`fp` в области времени** — тому, что рулит на самом деле.

Все планировщики скрыты за общим интерфейсом `IPlanner`, а сервис `Planner` держит ровно один из них:
...
Все они выдают **кривизну**, а не угол. Именно поэтому их можно менять на ходу и сравнивать на одном беге:
в угол руля кривизна переводится один раз и дальше по цепочке — в `Control`.
...
Если окно подгонки короткое или зашумлённое, $\kappa$ получается **заниженной и запаздывающей**, и эту ошибку наследует вся упреждающая цепочка.
...
Должно получиться CTE ≈ 0.3 м и $\kappa$ около 0.02. На реальном беге сравните κ из `control/lane_keep_debug` с `yaw_rate / v`.

### 2. Предсказать траектории для пробного $\delta$

Игрушка катится **вдоль пути**, а не по часам. Для кандидата $\delta$:
...
Чем меньше, тем лучше. Слагаемые, в упрощённом виде (они из VisionPilot — оттуда игрушка и берёт начало):
...
Минимум ожидается около `scale=1` ($\delta\approx\delta_{\mathrm{ff}}$). Мысленно обнулите `W_DELTA` — и порядок может смениться в пользу $\delta$, который минимизирует чистую CTE.

### 4. Построить $\delta_{\mathrm{ff}}$

Упреждение — это то руление, которое нужно, **даже когда вы уже точно по центру**:
...
Сначала затравка, затем шаги по конечным разностям или градиенту вдоль последовательности $\delta$ (в C++ около 80 итераций). На практике, на дорожных бегах, спуск почти не сдвигает затравку — **какова затравка, таков и выход**.
...
двигает всю *последовательность* $\delta_0\ldots\delta_{N-1}$; не принимайте этот блок за указание на то,
сколько переменных у MPC в действительности.
```
...
Насыщение на ±300 cNm означает, что команду не может отработать **сама машина**, а не что стоимость просила больше. Отладочный флаг и индикация в интерфейсе — предел руления. Всей второй половине шага «δ → момент» посвящена отдельная глава: [Угловой контур](../Control/AngleControl.md).
...
модели, а их σ-взвешенное слияние, собранное в `laneLinesToPath`. Самому слиянию, двум отказам, от которых
оно спасает, и тому, почему здесь кроется половина смещения на дуге, посвящена отдельная глава:
[Путь разметки](./LanePath.md). Эта глава исходит из того, что опора уже готова.
