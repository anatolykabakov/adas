...
Сервис **`safety_warn`** выводит на экран **только иконки и текст** — он **не** тормозит и не рулит.
...
Первая версия включала FCW, когда желаемое ускорение IDM падало ниже −3 м/с², а AEB — ниже −5. Звучало
...
поворот требуют сильного торможения, и оба раньше зажигали предупреждение о столкновении. Урок выходит за
пределы этого проекта: **желаемый выход контроллера не является сигналом опасности.** В нём заложено всё, что
важно контроллеру, а предупреждение должно строиться из самой угрозы.

Поэтому правило стало опираться на саму угрозу — время до столкновения и требуемое замедление, вычисленные по
реально существующей цели, — а ускорение IDM осталось в сообщении только как отладочное поле.
...
Исправили переходом только на `lead0`. Отмечаю это отдельно, потому что тот же дефект прожил в продольном
планировщике ещё два месяца и обнаружился только тогда, когда планировщику дали действовать.
...
Вторая причина интереснее. LDW существует, чтобы предупреждать *сносящего водителя*. Но при включённом
ассистенте смещение, которое он измеряет, — это собственная ошибка слежения ассистента на дуге; система
предупреждала сама о себе и делала это в 82 % случаев, когда вообще подавала голос.

```{admonition} Дефолт в заголовке — ещё не решение
:class: warning
Скоростной гейт выше подняли в коде, но **не** в `assets/config.json`, а тот хранит своё значение и
перебивает его. Поэтому правка оказалась мёртвой, и следующий заезд воспроизвёл те же три ложных срабатывания
на 4.8–5.5 м/с. Теперь три теста проверяют, что поставляемый конфиг соответствует решениям, принятым на
дороге, а сборка отказывается компилировать конфиг, который не разбирается.
```
...
width: 95%
---
Два триггера: время до столкновения ниже 2.5 с или требуемое замедление выше 3.5 м/с².
```
...
width: 70%
---
На дуге край собственной полосы пересекает линию «прямо вперёд» — сравнение с осью помечает выезд,
которого нет.
```
...
(Настоящая семантика защёлки — в `WarningLatch`, эталон — в тестах; этот набросок показывает лишь, *зачем* нужен антидребезг.)
