...
reason the two readings looked like a contradiction. Our own `include/adas/panda/health.h` already carries both
...
| ~~cruise-button actuation after the fix~~ | **superseded 2026-08-28**: the button path is gone; speed now goes the standard way — planner (upstream long MPC) → acceleration → `ACC_06/07/02` behind `vehicle.long_control` and the panda's longitudinal flag. Proven in MetaDrive (five scenarios), not on this car (no radar). Old note kept for the numbers: | four plan defects and the actuator ceiling are fixed with tests, and replay over run 08-06 gives 1.5 tips/min against 73.1 — but no drive yet. Enable deliberately (`--set vehicle.cruise_buttons=true`), not by default. Watch: tips per minute in single digits, `brake_needed` share, and that the set speed never rises above where the driver put it |
