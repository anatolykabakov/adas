...
2. Прогнать `latency.py`, визуализатор бега, PlotJuggler — установить темп и сквозную задержку.
3. Перебрать параметры (`bag_config_sweep.py`) при **фиксированной** предполагаемой задержке зрения.
