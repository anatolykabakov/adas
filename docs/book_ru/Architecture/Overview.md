...
2. запускает инференс **Supercombo** на устройстве — на GPU через собранный `.thneed`, с ONNX Runtime как
   запасным путём;
3. планирует в кривизне и считает поперечную команду в нативном **C++**;
4. публикует `HCA_01` на CAN VW MQB через Panda — через интерфейс платформы, так что марка это одна
   реализация, а не архитектура;
5. пишет полный **бег** для офлайн-анализа.

Это исследовательский и учебный стек, не «автопилот»: у поперечного контура есть дорожное время, продольный
(планировщик → ускорение → ACC-кадры машины) пока доказан только в симуляторе.
...
|---|---|---|
| **Java** | камера / IMU / GPS, VisionPipeline, логгер, ZMQ | датчики, инференс, интерфейс, запись бегов |
| **C++** | `AdasApp`, `Planner`, `Control`, `Platform`, калибровка, … | алгоритмы и актуация |
...
Сторона C++ намеренно делит поперечный контур натрое: `Planner` решает, *какой формы траекторию ехать*,
`Control` — *какая команда её даёт*, `Platform` выносит команду на шину и единственный знает, что машина —
Volkswagen. Появление второй марки (Toyota TSS2) не изменило в первых двух ни строки; порядок описан в
`docs/PORTING.md`.

Дальше в этой части: [Middleware](./Middleware.md) (нативная шина) → [Слой Java](./JavaLayer.md) (камера /
инференс / ZMQ) → [Конвейер](./Pipeline.md) (кадр → HCA). Ещё [FCW / AEB / LDW](../Safety/Warnings.md) —
предупреждения без актуации.
...
Пройденная с инструктором, эта же таблица — безопасный порядок подъёма для нового телефона, машины или
студента: каждая стадия добавляет одну необратимую вещь, и у каждой есть проверка *до* следующей.
Стадия 2 хочет стабильных оверлеев и разумной задержки (`tools/latency.py`), стадия 3 — чистой
`middleware/stats` на дорожных темпах, стадия 4 — правдоподобного `vehicle/state` против приборки, а
стадия 5 — одна строка здесь и целый документ на практике: `docs/PREDRIVE.md`, фиксирующий единственный
вопрос заезда *до* взгляда на данные. Дорожная работа — только под присмотром.
...
| ключ | назначение |
|---|---|
| `vision.model_runner` | `thneed` (GPU, по умолчанию) \| `onnx` (запасной) |
| `vehicle.name` | какой `CarPlatform` создаётся |
...
4. И только потом открывать `services/planner.cpp` / `lateral/pp_planner.cpp` с главами по управлению в руках.
