...
Неверный знак $y$ или неверный `steer_sign` **инвертируют руление**. Приведите договорённости в порядок до всякой математики контроллера.
...
width: 90%
---
Всегда спрашивайте, в каком $y$ построен график.
```

```{admonition} Запомнить
:class: tip
Supercombo: $y$ вправо. Машина по ISO: $y$ влево. Смешать их без явного переворота — значит поменять лево и право.
```
...
(знак зависит от того, значит ли положительная CTE «машина левее пути» или наоборот — **выберите одно и держитесь**).
...
Перевернуть **только** $y$ или **только** `steer_sign` — всё равно получить инвертированный HCA. Согласованы должны быть оба.

## Пиксель против метра

Пиксель — это **луч**, а не дальность. AAD восстанавливает метры через IPM.
Здесь Supercombo уже отдаёт метры; экстринсики всё равно важны, потому что они задают **warp входа**. Неверный тангаж → смещённые поперечные величины → постоянная CTE.

## Готовый чеклист

| наблюдение (device, $y$ вправо) | что значит |
|---|---|
| левая разметка $y\approx -1.8$ м | как ожидается |
...
1. Запустите примеры; меняйте знаки, пока `left_line_ok` не начнёт возвращать ложь — именно на эту ошибку студенты натыкаются в бегах.
2. В визуализаторе одним предложением назовите систему координат графика сверху.
3. Предскажите офлайн: переверните в конфиге только `steer_sign` и прогоните реплей PP — в какую сторону пойдёт руль?
