...
## Соберём сами: формат в сорок строк

Бег — это каталоги топиков с записями, и перед каждой записью стоит её длина. В этом вся идея; остальное —
protobuf. Напишите игрушечную версию, чтобы в настоящей не осталось загадок:
...
На трёх вещах спотыкается каждый — по разу, — поэтому лучше знать о них заранее.

**Передавайте `Path`, а не строку.** `load_topic_messages` склеивает пути через `/`, поэтому `str` даёт
`TypeError: unsupported operand type(s) for /`. Задним числом очевидно, а в момент сбивает с толку.

**Не выравнивайте сканированием.** Вспомогательная `nearest(ts, series)` — это линейный поиск: для пары
обращений нормально, а на весь бег — уже квадратично. Выровнять так 22 000 кадров зрения по 140 000 отсчётов
шасси таким способом — это часы работы. Отсортируйте один раз и ищите двоичным поиском.
...
На настоящих бегах это даёт 1.0117 и 1.0120 на двух разных заездах и ровно по всем диапазонам. Так и была
найдена ошибка радиуса колеса, описанная в главе [про локализацию](../Localization/Overview.md); тем же
...
выравниванием. Указывайте, сколько осталось. «Масштаб 1.012» по восьми отсчётам и по 798 — это разные
утверждения, и первое здесь уже однажды оказалось неверным.
```

## Запишите собственный бег и прогоните по нему собственный код

Для первого бега машина не нужна: лучше всего — телефон на лобовом стекле, когда вы едете пассажиром, но
сгодится и прогулка вдоль дороги с окрашенными краями. Соберите (`./scripts/build_project.sh`), поставьте,
включите логгирование из UI, подвигайтесь 3–5 минут, заберите сессию (`./scripts/pull_bags.sh`), откройте в
визуализаторе. То, чего в вашем беге быть не может — `vehicle/state`, `can/rx`, момента, — это как раз
достоинство: всё, что контроллер на нём выдаёт, проверить нельзя, так что ошибаться можно сколько угодно и без
последствий.
...
Ждите совпадения формы, но не чисел: σ ничем не похожа на синтетические 5 см (медианы 0.37/0.51 м на заездах
проекта, обе линии уверены одновременно лишь в ~22 % кадров); целые неотмечаемые отрезки без обеих линий;
шаг кадров 42 мс с дрожанием. Запишите три худших момента — пропуск, вспышку σ, скачок шага кадров — с меткой
времени и одним предложением на каждый. Эти три метки с этого дня станут вашим личным тестовым набором.
