...
происходит до [контроллеров](./MPC_and_FP.md), и здесь решается примерно половина измеренного смещения
от центра.
...
|---|---|---|---|
| **линии разметки** | `lanes[1]` (ближняя левая), `lanes[2]` (ближняя правая) | геометрический центр *этой* полосы | одна пропадает; σ взрывается на дугах и в дождь |
| **план модели** | `plan_x` / `plan_y`, собственный путь сети | всегда есть, всегда гладкий | **срезает внутрь** дуги — систематическое смещение уставки |

Линии правы, когда есть, и лгут, когда пропадают. План есть всегда и всегда чуть неправ на дугах.
Слияние — не «на всякий случай», а использование каждого ровно там, где отказывает другой.
...
1. **вероятность**, смягчённая, чтобы линия чуть выше порога не прыгала на полный вес:
...
2. **σ-уверенность** — полный вес до `lane_std_good_m` (0.15 м), линейно до нуля к `lane_std_bad_m`
   (0.30 м). Это собственные полосы `laneLineStds` comma: ниже 0.15 м модель уверена настолько,
   насколько вообще бывает, выше 0.30 м — гадает:
...
3. **ширина** — паре доверяют (*заякорена*) только когда обе линии прошли и их медианная разница лежит
   в [2.6, 4.6] м. Стык гудрона или полоса слияния обычно ломают ширину, так что одна дешёвая проверка
   отсекает самый частый вход «уверенно, но неверно».
...
width: 90%
---
На дуге план модели срезает внутрь; разметка сидит на истинном центре. `blend=0` едет по плану,
...
width: 80%
---
Вес подмешивания по мере роста σ: за `lane_std_bad_m` (0.30 м) линии получают нулевой вес, а 0.37/0.51 м с
...
* перебор подмешивания воспроизводится: `blend=0` едет по плану, `blend=1` по разметке, `blend=0.6`
  между;
* перебор σ показывает, как `d_prob` уходит в ноль, едва σ переваливает 0.30 м, — так что, зная 0.37 и
  0.51 м с заезда, вы можете объяснить, почему линии обычно не участвуют;
* одно предложение, связывающее долю «обе уверены» ~22 % со смещением на дуге.

## Куда дальше

* [Зрение — обзор](../Vision/Overview.md) — откуда берутся σ и план, и четыре условия к опоре.
